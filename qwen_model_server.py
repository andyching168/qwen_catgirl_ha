...
3. area 是區域名稱（例如：書房、客廳、臥室），如果使用者沒有說區域的話，直接用書房
4. 純聊天時不輸出 ACTION
5. 回應要親切可愛，適當加入「喵」
...
def run_inference(user_input: str, device_list: str) -> str:
    ...
    if has_control_keyword:
        logger.info("🔧 檢測到控制關鍵字，使用低 temperature (0.3)")
        print("🔧 檢測到控制關鍵字，使用低 temperature")
        initial_temp = 0.3
    else:
    ...
    # Add current date/time to the prompt sent to the model
    messages = [
    ...
    # 如果沒有控制關鍵字 且 沒有 ACTION，用高 temp 重新生成
    if not has_control_keyword and "ACTION" not in response:
...
async def root():
    return {
        "status": "ok", 
        "message": "Qwen Catgirl Home Assistant API v6",
        "version": "v6 (name + area)",
        "model": MODEL_PATH
    }
...
async def process_request(request: InferenceRequest):
    ...
    print(f"🔧 設備數量: {len(request.devices)}")
    
    logger.info("=" * 60)
    logger.info(f"📨 收到 /process 請求")
    logger.info(f"💬 使用者輸入: {request.text}")
    logger.info(f"🔧 設備數量: {len(request.devices)}")
    ...
    try:
        ...
        if TIME_QUESTION_PATTERN.search(request.text):
            ...
            # 執行推理（一般流程）
            response = run_inference(request.text, device_list)
